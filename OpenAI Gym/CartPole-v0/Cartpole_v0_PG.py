...
class RLAgent:
    ...
    def build_model(self):
        ...
        with tf.name_scope("Layer_1"):
            weights = tf.compat.v1.Variable(tf.random.normal([self.state_size, 20]))
        ...
        with tf.name_scope("Layer_2"):
            weights = tf.compat.v1.Variable(tf.random.normal([20, 12]))
            biases = tf.compat.v1.Variable(tf.zeros([12]))
            tf.compat.v1.summary.histogram("weights", weights)
            tf.compat.v1.summary.histogram("biases", biases)
        ...
        with tf.name_scope("Output"):
            weights = tf.compat.v1.Variable(tf.random.normal([12, self.action_size]))
